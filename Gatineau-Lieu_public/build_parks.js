...
console.log('Clusters:', newExtents.all().features.length, 'Parks:', newTree.all().features.length)

const osm = geojson2osm.geojson2osm(newTree.all())
fs.writeFileSync('gatineau-new-parks.osm', osm);
fs.writeFileSync('gatineau-new-parks_clusters.geojson', JSON.stringify(newExtents.all(), null, 4));